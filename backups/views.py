...
# cur = conn.cursor()
from .models import Account, Campaign, Location
...
def get_all_data():
        ...
        for row in get_client_accounts():
            ...
            for camp in get_account_campaigns(row['customer_id']):
                ...
                results.append({'account_name': row['name'], 'account_id': row['customer_id'], 'campaign_name': camp['name'], 'campaign_id': camp['id'],'status': row['status'], 'conversions': c, 'locations': str(unique_geo_constants)})
